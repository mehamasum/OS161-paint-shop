...
void customer(void *unusedpointer, unsigned long customernum)
{
  ...
  do {
    ...
    can.requested_colours[1] = GREEN;
    can.requested_colours[2] = YELLOW;
...
int runpaintshop(int nargs, char **args)
{
  ...
  for (i=0; i<NPAINTSHOPSTAFF; i++) {
    result = thread_fork("paint shop staff thread", NULL, paintshop_staff, NULL, i);
    if (result) {
  ...
  for (i=0; i<NCUSTOMERS; i++) {
    result = thread_fork("customer thread", NULL, customer, NULL, i);
    if (result) {
      panic("runpaintshop: thread_fork failed: %s\n",
	    strerror(result));
    }
